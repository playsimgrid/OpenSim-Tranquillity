        ...
        public IPEndPoint RemoteIPEndPoint
        {
            get
            {
                if(m_remoteIPEndPoint == null)
                {
                    ...
                    // remoteEndPoint of the connection ("client that connected"), so it IS the
                    // real socket peer. Before this check, an x-forwarded-for header from ANY
                    // peer replaced that peer for every IP-based decision on the public ports:
                    // bans, logging, and any source-address allowlist built on this property.
                    //
                    // TrustedForwardedProxies is EMPTY by default, which means the header is
                    // ignored. That is the right default: a grid with no reverse proxy - the
                    // common case, and ours - has nothing that legitimately sets this header,
                    // so anything that does is lying. Operators who do front ROBUST with a
                    // proxy list its addresses in config.
